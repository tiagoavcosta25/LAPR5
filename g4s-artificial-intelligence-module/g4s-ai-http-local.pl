...
best_prepareEmotions(Request, Path, Cost) :-
    http_parameters(Request, [emailPlayer(EmailPlayer, [string]), emailTarget(EmailTarget, [string]), n(N, [integer]), mode(Mode, [number]), joy(Joy, [number]), anguish(Anguish, [number]), hope(Hope, [number]), deception(Deception, [number]), fear(Fear, [number]), relief(Relief, [number])]),
	addPlayers(),
	addConnections(),
	getPlayerName(EmailPlayer, PlayerName),
	getPlayerName(EmailTarget, TargetName),
