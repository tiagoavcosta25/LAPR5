    ...
    public class ProfileService
    {
        ...
        public async Task<List<ProfileDto>> GetAllAsync()
        {
            ...
            List<ProfileDto> listDto = list.ConvertAll<ProfileDto>(prof => 
                new ProfileDto(prof.Id.AsGuid(),prof.Name.name, prof.Email.address, prof.PhoneNumber.phoneNumber, prof.DateOfBirth.date.Year, prof.DateOfBirth.date.Month, prof.DateOfBirth.date.Day));
        ...
        public async Task<ProfileDto> GetByIdAsync(ProfileId id)
        {
            ...
            return new ProfileDto(prof.Id.AsGuid(),prof.Name.name, prof.Email.address, prof.PhoneNumber.phoneNumber, prof.DateOfBirth.date.Year, prof.DateOfBirth.date.Month, prof.DateOfBirth.date.Day);
        }

        public async Task<ProfileDto> AddAsync(CreatingProfileDto dto)
        {
            var Profile = new Profile(dto.Name, dto.Email, dto.PhoneNumber, dto.DateOfBirth.Year, dto.DateOfBirth.Month, dto.DateOfBirth.Day);

            await this._repo.AddAsync(Profile);

            await this._unitOfWork.CommitAsync();

            return new ProfileDto(Profile.Id.AsGuid(),Profile.Name.name, Profile.Email.address, Profile.PhoneNumber.phoneNumber, Profile.DateOfBirth.date.Year, Profile.DateOfBirth.date.Month, Profile.DateOfBirth.date.Day);
        }
